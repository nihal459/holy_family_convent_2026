...
def news_announcements(request):
    ...
    if request.method == 'POST':
        ...
        elif attachment and link:
            error = "Please provide either an attachment or a link, not both."
        elif attachment and attachment.size > 2 * 1024 * 1024:
            error = "Attachment must not exceed 2 MB. For larger files, please use the link option instead."
...
def news_announcements_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        new_link = request.POST.get('link', '').strip() or None

        error = None
        if new_attachment and new_link:
            error = "Please provide either an attachment or a link, not both."
        elif new_attachment and new_attachment.size > 2 * 1024 * 1024:
            error = "Attachment must not exceed 2 MB. For larger files, please use the link option instead."
        ...
        if error:
            ...
            if new_attachment:
                news.attachment = new_attachment
                news.link = None
            elif new_link:
                news.link = new_link
